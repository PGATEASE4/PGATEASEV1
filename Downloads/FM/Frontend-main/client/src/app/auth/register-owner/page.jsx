...
export default function OwnerRegistrationPage() {
  ...
  const register = async () => {
    ...
    try {
      ...
      // Step 3: Add user to owner team (now authenticated)
      await teams.createMembership(
        process.env.NEXT_PUBLIC_TEAM_OWNER || 'pgateaseteam2owner',
        email,
        ['owner'],
        process.env.NEXT_PUBLIC_VERIFY_URL || 'https://pgateasev-1.vercel.app/auth/verify'
      );

      // Step 4: Create user document with owner permissions
      await databases.createDocument(
        ...
        [
          ...
          // Residents can read owners (for contact info)
